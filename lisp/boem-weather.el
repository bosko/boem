;;; boem_weather.el --- Dissplay weather from api.open-meteo.com -*- lexical-binding: t; -*-
;;
;; Author: Boško Ivanišević

;;; Commentary:
;;
;; Displays current, hourly or daily forecast in a pop-up frame
...
(defun boem-weather--bottom-line ()
  (concat
   ...
   " hourly "
   (propertize "any" 'face 'help-key-binding)
   " quit"
   ))
...
(defun boem-weather--fmt-hours (time)
  (let* ((cur-hour (format-time-string "%H"))
    ...
    (if (string-equal cur-hour time-hour)
        (propertize (substring time 11 16) 'face 'font-lock-string-face)
      (propertize (substring time 11 16) 'face 'bookmark-face)
        )
    )
  )
...
(defun boem-weather--fmt-daily (data)
  ...
  (let* ((daily (gethash "daily" data))
         ...
         (result-lines nil))
    (push (propertize "Daily forecast" 'face '(bold underline)) result-lines)
    (push "" result-lines)
    (dotimes (i 7)
...
(defun boem-weather--fmt-hourly (data)
  (let* ((hourly (gethash "hourly" data))
         ...
         (result-lines nil))
    (push (propertize "Hourly forecast" 'face '(bold underline)) result-lines)
    (push "" result-lines)
